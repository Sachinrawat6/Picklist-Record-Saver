...
const Orders = () => {
  ...
  const updateSidebarCounts = (orders, channel) => {
    ...
    setSidebarCounts({
      ...
      found: filteredByChannel.filter(
        (o) => o.status?.toLowerCase() === "found"
      ).length,
      cutting: filteredByChannel.filter(
        (o) => o.status?.toLowerCase() === "cutting"
      ).length,
      alter: filteredByChannel.filter(
        (o) => o.status?.toLowerCase() === "alter"
      ).length,
      no_fabric: filteredByChannel.filter(
        (o) => o.status?.toLowerCase() === "no fabric"
      ).length,
    });
  };
  ...
  useEffect(() => {
    const filtered = orders.filter((order) => {
      const matchesSearch = order.style_number
        ?.toString()
        .toLowerCase()
  ...
  const downloadMRPTags = () => {
    ...
    link.setAttribute(
      "download",
      `MRP_Tags_${new Date().toISOString().split("T")[0]}.csv`
    );
  ...
  const downloadPackingList = () => {
    ...
    const ordersToExport = orders.filter(
      (order) =>
        !order?.status.toLowerCase().includes("cutting") &&
        !order?.status.toLowerCase().includes("nofab")
    );
  ...
  const downloadQRCodeSheet = () => {
    ...
    const foundOrders =
      syncOrders?.cutting_list.length > 0 ? syncOrders?.cutting_list : [];
    ...
    link.setAttribute(
      "download",
      `QR_CODE_SHEET${new Date().toISOString().split("T")[0]}.csv`
    );
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  ...
  const exportToPDF = (data, patternData, fileName = "Cutting_List") => {
    ...
    data
      ...
      .forEach((order) => {
        ...
        const patternInfo =
          patternData.find((p) => p.style_number === style) || {};
    ...
    const totalRow = [
      ...
      ...sizes.map((size) => {
        return tableData.reduce(
          (sum, row) => sum + (parseInt(row[sizes.indexOf(size) + 2]) || 0),
          0
        );
      }),
      tableData.reduce(
        (sum, row) => sum + (parseInt(row[sizes.length + 2]) || 0),
        0
      ),
    ];
  ...
  //   const picklist_id_exists = syncLogData.find(
  //     (id) => Number(id.picklist_id) === Number(picklistId)
  //   );
  //   // const alter = syncLogData.find((id) =>
  //   //   id.status.toLowerCase().includes("alter")
  //   // );
  //   // const no_fabric = syncLogData.find((id) =>
  //   //   id.status.toLowerCase().includes("no fabric")
  //   // );
  //   // const cutting = syncLogData.find((id) =>
  //   //   id.status.toLowerCase().includes("cutting")
  //   // );

  //   if (picklist_id_exists) {
  //     setError("Picklist already synced. Please sync another picklist");
  //     return;
  //   }
  ...
  //       status: order?.status || "",
  //       found_in_inventory:
  //         order.status?.toLowerCase().includes("found")? true : false,
  //     }));
  ...
  const syncOrdersToFastApi = async () => {
    ...
    const picklist_id_exists = syncLogData.find((id) => Number(id.picklist_id) === Number(picklistId));
  ...
  if (error) {
    return (
      <div
        className="bg-red-100 container mx-auto border border-red-400 text-red-700 px-4 py-3 rounded relative mt-17 "
        role="alert"
      >
        ...
        <div className="mt-2 ">
          {" "}
          <button
            className="bg-black  py-2 px-4 text-white rounded shadow"
            onClick={() => {
              window.location.reload();
            }}
          >
  ...
  if (isSyncing) {
    return (
      <div className="fixed  left-0 right-0 bg-white z-50 p-4 shadow-md">
        <div className="container mx-auto">
          <div className="flex items-center justify-between mb-2">
            <span className="font-medium">Syncing orders...</span>
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      ...
      <div className="w-64 bg-white mt-15 p-4 border-r border-gray-200 fixed h-full overflow-y-auto">
          ...
          <ul className="space-y-1">
            {[
              { label: "All Orders", value: "all", count: sidebarCounts.all },
              {
                label: "Found",
                value: "found",
                count: sidebarCounts.found,
                color: "green",
              },
              {
                label: "Cutting",
                value: "cutting",
                count: sidebarCounts.cutting,
                color: "yellow",
              },
              {
                label: "Alter",
                value: "alter",
                count: sidebarCounts.alter,
                color: "blue",
              },
              {
                label: "No Fabric",
                value: "no fabric",
                count: sidebarCounts.no_fabric,
                color: "red",
              },
            ].map((item) => (
              <li key={item.value}>
        ...
        {orders.length > 0 && syncOrders.length > 0 ? (
          <>
