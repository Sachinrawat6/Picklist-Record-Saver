          ...
          <tbody className="divide-y divide-gray-100 bg-white">
            {syncLogData.length > 0 ? (
              syncLogData.map((record, i) => (
                <tr key={record.id || i} className="hover:bg-gray-50 transition">
                  <td className="px-6 py-4 text-gray-900">{i + 1}</td>
                  <td className="px-6 py-4 text-gray-700">{record.channel || "-"}</td>
                  <td className="px-6 py-4 text-gray-700">{record.picklist_id || "-"}</td>
                  <td className="px-6 py-4 text-gray-700">{record.sync_id || "-"}</td>
                </tr>
              ))
            ) : (
              <tr>
